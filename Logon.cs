...
namespace CyberArk.Extensions.Identity.AdmAccts
{
    public class Logon : BaseAction
    {
        ...
        override public int run(ref PlatformOutput platformOutput)
        {
            ...
            try
            {
                ...
                string username = ParametersAPI.GetMandatoryParameter(USERNAME, LogOnAccount.AccountProp);
                ParametersAPI.ValidateParameterLength(username, "Username", 64);

                string address = ParametersAPI.GetMandatoryParameter(ADDRESS, LogOnAccount.AccountProp);
                ParametersAPI.ValidateURI(address, "Address", UriKind.RelativeOrAbsolute);

                string identityScope = ParametersAPI.GetMandatoryParameter(IDENSCOPE, LogOnAccount.AccountProp);
                ParametersAPI.ValidateAlphanumeric(identityScope, "Identity Scope");

                string identityAppId = ParametersAPI.GetMandatoryParameter(IDENAPPID, LogOnAccount.AccountProp);
                ParametersAPI.ValidateAlphanumeric(identityAppId, "Identity Application Id");
                #endregion

                #region Fetch Account's Passwords
                SecureString secureCurrPass = LogOnAccount.CurrentPassword;
